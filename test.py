...
model = sm.ARIMA(spx['Return'], order=(1, 0, 1)).fit()
resid = model.resid
...
best_model = arch_model(resid, vol='GARCH', p=int(best_aic_model['p']), q=int(best_aic_model['q']), dist='normal')
...
plt.plot(spx["Date"], annual_vol, label='Conditional Volatility')
plt.title(f'GARCH({int(best_aic_model["p"])},{int(best_aic_model["q"])}) Conditional Volatility')
plt.xlabel('Time')
